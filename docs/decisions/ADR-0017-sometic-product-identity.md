...
Sometic is the public portable application behavior product under the Sometic parent brand. Identity must be locked before first publish so packages, custom elements, docs, and tooling share one name and scope.
...
5. Product docs URL: **`https://sometic.dev`**.
6. GitHub repository target: **`aitistack/sometic`** (until `set-repo-identity` overrides).
7. Public site and brand attribution use **Sometic** only (`sometic.dev`).
...
- DNS for `sometic.dev` must be configured for the live docs hostname.
...
- Docs scope check rejecting stale `@sometic/` and `@sometic-ui/` in consumer markdown
